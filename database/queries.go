...
func CreateRingtone(name string, category int, phones []int, effect int, authorID int, autoGenerated bool, glyphData string, hash []byte) (string, error) {
	...
	var ringtoneID int
	err = DB.QueryRow(`INSERT INTO ringtone (display_id, name, category, effect_id, author_id, auto_generated, glyphs, hash) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id;`, displayID, name, category, effect, authorID, autoGenerated, glyphData, hash).Scan(&ringtoneID)
	if err != nil {
		return "", err
	}
...
func GetRingtone(displayID string, authorID int) (RingtoneModel, error) {
	...
	rows, err := DB.Query(`SELECT r.id, r.display_id, r.name, ARRAY ( SELECT p.name FROM phone_and_ringtone par INNER JOIN phone p ON p.id = par.phone_id WHERE par.ringtone_id = r.id ) as phone_names, u.id AS author_id, u.name AS author_name, e.name AS effect_name, r.category, r.downloads, COUNT(*) FILTER ( WHERE v.up IS TRUE ) - COUNT(*) FILTER ( WHERE v.up IS FALSE ) AS votes, ( CASE WHEN $2 = 0 THEN 0 WHEN ( SELECT up FROM vote WHERE author_id = $2 AND ringtone_id = r.id ) IS TRUE THEN 1 WHEN ( SELECT up FROM vote WHERE author_id = $2 AND ringtone_id = r.id ) IS FALSE THEN 2 ELSE 0 END ) AS user_vote, r.glyphs FROM ringtone r INNER JOIN effect e ON r.effect_id = e.id INNER JOIN author u ON r.author_id = u.id FULL OUTER JOIN vote v ON r.id = v.ringtone_id WHERE r.display_id = $1 GROUP BY ( r.id, r.display_id, r.name, u.id, u.name, e.name, r.category, r.downloads );`, displayID, authorID)
	if err != nil {
