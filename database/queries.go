...
func CreateRingtone(name string, category int, phones []int, effect int, authorID int, autoGenerated bool, glyphData string) (int, error) {
...
func GetRingtoneByDisplayID(displayID string) (RingtoneModel, error) {
	var ringtone RingtoneModel
	rows, err := DB.Query(`SELECT r.id, r.display_id, r.name, ARRAY ( SELECT p.name FROM phone_and_ringtone par INNER JOIN phone p ON p.id = par.phone_id WHERE par.ringtone_id = r.id ) as phone_names, u.id AS author_id, u.name AS author_name, e.name AS effect_name, r.downloads FROM ringtone r INNER JOIN effect e ON r.effect_id = e.id INNER JOIN author u ON r.author_id = u.id WHERE r.display_id = $1;`, displayID)
	if err != nil {
		log.Println(err.Error())
		return ringtone, err
	}
